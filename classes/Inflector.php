...
//			'person' => 'people',
		...
		if(isset(self::$cache['pluralized'][$word])) {
//			echo "hit cache $word<hr>";
		...
		foreach ($irregular as $_plural => $_singular) {
			if (preg_match('/(' . $_plural . ')$/i', $word, $arr)) {
				$ret = preg_replace('/(' . $_plural . ')$/i', substr($arr[0], 0, 1) . substr($_singular, 1), $word);
				self::$cache['pluralized'][$word] = $ret;
				return $ret;
			}
		}
...
//				echo $rule . "," . $word;
				$ret = preg_replace($rule, $replacement, $word);
				self::$cache['pluralized'][$word] = $ret;
				return $ret;
			}
		}
	...
	static function singularize($word)
	{
		...
		if(isset(self::$cache['singularized'][$word])) {
//			echo "hit sing " . $word . "<hr>";
		...
		foreach ($irregular as $_plural => $_singular) {
			if (preg_match('/(' . $_singular . ')$/i', $word, $arr)) {
				self::$cache['singularized'][$word] = preg_replace('/(' . $_singular . ')$/i', substr($arr[0], 0, 1) . substr($_plural, 1), $word);
				return self::$cache['singularized'][$word];
			}
		}
		...
		foreach ($singular as $rule => $replacement) {
			if (preg_match($rule, $word)) {
				self::$cache['singularized'][$word] = preg_replace($rule, $replacement, $word);
				return self::$cache['singularized'][$word];
			}
		}
	...
	{
		if(isset(self::$cache['camelized'][$word])) {
//			echo "hit sing " . $word . "<hr>";
			return self::$cache['camelized'][$word];
		}
		self::$cache['camelized'][$word] = str_replace(' ', '', ucwords(preg_replace('/[^A-Z^a-z^0-9]+/', ' ', $word)));
		return self::$cache['camelized'][$word];
	}
	...
	static function underscore($word)
	{
		...
		if(isset(self::$cache['underscored'][$word])) {
//			echo "hit sing " . $word . "<hr>";
			return self::$cache['underscored'][$word];
		}
		self::$cache['underscored'][$word] = strtolower(preg_replace('/[^A-Z^a-z^0-9]+/', '_',
